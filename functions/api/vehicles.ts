    ...
    if (existing && env.DISCORD_WEBHOOK_APPLICATIONS) {
      ...
      const embedPayload: any = isTrailer ? {
        ...
        fields: [
          ...
          { name: "トレーラー", value: `${maker ? maker + ' ' : ''}${model}`, inline: false },
          { name: "種別", value: existing.trailer_type || '-', inline: true },
          { name: "カラー", value: color || '-', inline: true },
          { name: "ナンバープレート", value: `[${plate_region}] ${plate}`, inline: true }
        ],
        timestamp: new Date().toISOString(),
        ...
        fields: [
          ...
          { name: "更新後トリム / カラー", value: `${trim || '-'} / ${color || '-'}`, inline: true },
          { name: "更新後ナンバープレート", value: `[${plate_region}] ${plate}`, inline: true }
        ],
        timestamp: new Date().toISOString(),
      };
...
export const onRequestPatch = async ({ env, request }: { env: any, request: Request }) => {
  ...
  if (!['approved', 'pending', 'rejected', 'approved_warning', 'temp_approved'].includes(status) || !expected_status) {
  ...
  try {
    ...
    if (status === 'temp_approved') {
      ...
      const expiresAt = now.toISOString().replace('T', ' ').substring(0, 19);
    ...
    if (v && env.DISCORD_WEBHOOK_RESULTS) {
      ...
      if ((status === 'rejected' || status === 'approved_warning') && reject_reason) {
        fields.push({ name: status === 'rejected' ? "却下理由" : "非推奨理由", value: reject_reason, inline: false });
      }
    ...
    if (v && v.owner_id) {
      ...
      if (title && bodyText) {
        await sendFcmNotificationToUser(env, v.owner_id, {
          ...
          data: { 
            action: 'garage',
            updateType: 'vehicle_application',
            status: status
          }
